...
class Control:
    def __init__(self, page: Page, md_path):
        # 初始化各功能组件
        self.Zhihu_Login = Login(page=page)
        self.Zhihu_GetHot = GetHot(page=page)
        self.Zhihu_SendEssay = SendEssay(page=page)
        self.Zhihu_WriteText = WriteZhihuText(model_name="deepseek-chat")
        self.str_2_md = Str2Md()
        self.log = LoggingConfig(log_file_path=config.logfile_path).get_logger("Control")
    ...
    def __sanitize_filename(self, title: str) -> str:
        ...
        # 限制文件名长度（Windows 最大 255 字符，但保留扩展名和其他部分）
        max_length = 200
